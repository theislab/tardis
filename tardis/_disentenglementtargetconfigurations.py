...
from pydantic import BaseModel, StrictBool, StrictStr, StrictFloat, StrictInt, field_validator  # ValidationError
from typing import List, Optional

class TardisLossConfig(BaseModel):
    apply: StrictBool
    method: StrictStr
    weight: StrictFloat
...
class CounteractiveMinibatchGenerator(BaseModel):
    method: StrictStr
    # Accepts any dict without specific type checking. 
    method_kwargs: dict
...
class AuxillaryLosses(BaseModel):
    loss_complete_latent: TardisLossConfig
    loss_subset_latent: TardisLossConfig

class DisentenglementTargetConfiguration(BaseModel):
    obs_key: StrictStr
    n_reserved_latent: StrictInt
    counteractive_minibatch_generator: CounteractiveMinibatchGenerator
...
def _disentenglement_target_configurations_example_usage():
    ...
    _validated_data_empty = DisentenglementTargetConfigurations(**config_data_empty)
    ...
    _validated_data_filled = DisentenglementTargetConfigurations(**config_data_filled)
