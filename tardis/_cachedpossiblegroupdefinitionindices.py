...
class DatapointDefinitionsKeyGenerator:
    ...
    def create_definitions(cls, dict_items, target_obs_key_ind, config):
        ...
        if config.method_kwargs["within_other_groups"] and operation_mode_torch:
            ...
            group_definitions = dict_items[REGISTRY_KEY_DISENTENGLEMENT_TARGETS].iloc[:, target_obs_key_ind].values.reshape(-1, 1)
        elif operation_mode_torch:
        ...
        if REGISTRY_KEYS.CAT_COVS_KEY in dict_items:
            ...
            for include_cotcov_ind, include_cotcov in enumerate([] if cmk is None else cmk):
                if include_cotcov and operation_mode_torch:
                    catcovs_definitions.append(dict_items[REGISTRY_KEYS.CAT_COVS_KEY][:, include_cotcov_ind].view(-1, 1))
                elif include_cotcov:
                    catcovs_definitions.append(dict_items[REGISTRY_KEYS.CAT_COVS_KEY].iloc[:, include_cotcov_ind].values.reshape(-1,1))
                elif operation_mode_torch:
...
class CachedPossibleGroupDefinitionIndices:
    ...
    def _initialize_verify_input(dataset_tensors):
        ...
        known_keys = dict(
            must_have_keys = [REGISTRY_KEY_DISENTENGLEMENT_TARGETS, REGISTRY_KEYS.X_KEY, REGISTRY_KEYS.BATCH_KEY, REGISTRY_KEYS.LABELS_KEY],
    ...
    def _initialize_verify_config(config):
        ...
        if n_cmk != n_ecc:
            raise ValueError(
                "Number of categorical covariate should be the same as the number of elements in `within_categorical_covs`."
            )
    ...
    def _initialize(cls, dataset_tensors, target_obs_key_ind, data_split_identifier, splitter_index, config):
        ...
        for unique_definition_tuple in obs_key_items:
            obs_key_items[unique_definition_tuple] = np.intersect1d(
                np.array(obs_key_items[unique_definition_tuple]),
                splitter_index  # only keep the one in the data split
            )
        for unique_definition_tuple in obs_key_items:
    ...
    def get(cls, dataset_tensors, target_obs_key_ind, data_split_identifier, splitter_index, config):
        try:
            ...
            lengths_to_report = ','.join([str(len(cls._items[data_split_identifier][target_obs_key_ind][c])) for c in cls._items[data_split_identifier][target_obs_key_ind].keys()])
            warnings.warn(
