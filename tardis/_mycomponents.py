...
class FCLayersReservedLatentInjection(FCLayers):
            ...
            collections.OrderedDict(
                [
                    (
                        ...
                        nn.Sequential(
                            nn.Linear(
                                n_in + (cat_dim + n_reserved) * self.inject_into_layer(i),
                                n_out,
                                bias=bias,
                            ),
                ...
                if layer is not None:
                    if isinstance(layer, nn.BatchNorm1d):
                        ...
                        if isinstance(layer, nn.Linear) and self.inject_into_layer(i):
                            ...
                            x = torch.cat((x, reserved_latent_injection, *one_hot_cat_list_layer), dim=-1)
                        x = layer(x)
        return x
    ...
    def set_online_update_hooks(self, hook_first_layer=True):
        ...
        def _hook_fn_weight(grad):
            categorical_dims = sum(self.n_cat_list) + self.n_reserved
...
class DecoderSCVIReservedLatentInjection(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.px_decoder = FCLayersReservedLatentInjection(
            n_in=n_input,
            n_out=n_hidden,
