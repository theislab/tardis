...
def check_wandb_configurations(wandb_configurations):
    ...
    if wandb.api.api_key is None:
        raise ValueError(
            "Not logged in to Weights & Biases. No API Key found. Please login prior to the training."
        )
    ...
    for verify_key in ["username", "email"]:
        if si[verify_key] != wandb_configurations["login_credentials"][verify_key]:
            raise ValueError(
                f"Current `wandb` login conflicts with wandb configuration YAML file: `{verify_key}`"
            )

    try:
        possible_entities = [
            i["node"]["name"]
            for i in wandb.api.viewer_server_info()[0]["teams"]["edges"]
        ]
    except KeyError:
