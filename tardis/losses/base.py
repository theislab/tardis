...
class TardisLoss(nn.Module, ABC):
    ...
    def __init__(
        ...
        if isinstance(coefficient, float):
            self.coefficient_fn = lambda x, other: coefficient * torch.ones(x.shape[0], device=x.device)
    ...
    def _validate_forward_inputs(
        ...
        if not (isinstance(outputs, dict) and "z" in outputs.keys() and "qz" in outputs.keys()):
        ...
        if not (
            ...
            raise ValueError("counteractive_outputs should be a dictionary with key 'z'")
        ...
        if not relevant_latent_indices.shape[-1] <= outputs["z"].shape[-1]:
            raise ValueError("relevant_latent_indices should be the subset of indices of outputs['z']")
        if not counteractive_outputs["z"].shape == outputs["z"].shape:
            raise ValueError("counteractive_outputs['z'] should have the same shape as outputs['z']")
        ...
        if not len(relevant_latent_indices.shape) == 1:
            if len(relevant_latent_indices.shape) != 2:
                raise ValueError("relevant_latent_indices should be a 1D tensor or a 2D tensor with shape (1, n)")
    ...
    def forward(
        ...
        loss = self.weight * self.transformation(self._forward(outputs, counteractive_outputs, relevant_latent_indices))
        return loss
